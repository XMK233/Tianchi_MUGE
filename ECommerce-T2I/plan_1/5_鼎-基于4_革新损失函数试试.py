...
LEARNING_RATE = 5e-5
NUM_EPOCHS = 2
TRAIN_CHUNK_SIZE = 100  ## 训练集加载多少
TOTAL_IMAGES = 100 ## 训练样本总共加载多少
CHUNK_SIZE = 100  ## 验证集总共加载多少
...
def generate_images(model, vqgan, text_encoder, tokenizer, descriptions, num_samples=1):
    ...
    for i, desc in enumerate(tqdm(descriptions, desc="生成图像")):
        # 编码文本
...
if __name__ == "__main__":
    ...
    # 限制数量用于测试 - 减少到30张以降低内存压力
    descriptions = descriptions[:30]
    real_images = real_images[:30]
    ...
    for i, (img_tensor, desc) in enumerate(zip(generated_images, descriptions)):
        # 转换为PIL图像 - 移除批次维度
        img_pil = T.ToPILImage()(img_tensor.squeeze(0))
