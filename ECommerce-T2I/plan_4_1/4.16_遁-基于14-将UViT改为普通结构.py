...
class SimpleUNet(nn.Module):
    ...
    普通的 UNet 结构：
    - 替换了原有的 UViT 结构。
    - 使用 ResBlock 替代 ViT/MoE 瓶颈层。
    - 纯卷积架构，不包含 Attention。
    """
    def __init__(self, num_classes=1, class_emb_size=4, time_emb_size=32, text_emb_size=768):
        super().__init__()
        # 条件嵌入
        ...
        # 瓶颈层 (CNN)
        self.neck_proj = nn.Conv2d(128, 256, 1)
        self.cond_proj = nn.Linear(time_emb_size + class_emb_size, 256)
        
        self.mid_block1 = ResBlock(256)
        self.mid_block2 = ResBlock(256)
        self.mid_block3 = ResBlock(256)
        
        self.neck_out = nn.Conv2d(256, 128, 1)
    ...
    def forward(self, x, t, class_labels, text_emb):
        ...
        # 瓶颈层
        h_neck = self.neck_proj(p3)
        
        # 时间与类别条件 (Mid Fusion)
        if t.dim() == 0: t = t.view(1).repeat(bs)
        t = t.long().clamp(0, 999)
        t_feat = self.t_emb(t)
        c_feat = self.class_emb(class_labels)
        cond = torch.cat([c_feat, t_feat], dim=1)
        cond_emb = self.cond_proj(cond).view(bs, -1, 1, 1)
        
        h_neck = h_neck + cond_emb
        h_neck = self.mid_block1(h_neck)
        h_neck = self.mid_block2(h_neck)
        h_neck = self.mid_block3(h_neck)
        
        neck_out = self.neck_out(h_neck)

        # 解码器
        u1 = F.interpolate(neck_out, scale_factor=2, mode='bilinear', align_corners=False)
...
net = SimpleUNet(text_emb_size=hidden_size).to(device)
loss_fn = nn.MSELoss()
...
net_ema = SimpleUNet(text_emb_size=hidden_size).to(device)
