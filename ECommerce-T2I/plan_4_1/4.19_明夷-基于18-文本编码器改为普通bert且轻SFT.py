...
# 放开部分层进行训练（轻SFT：放开最后3层）
text_encoder.train()
for param in text_encoder.parameters():
    param.requires_grad = False

# 放开最后3层 encoder layer 和 pooler
for param in text_encoder.encoder.layer[-3:].parameters():
    param.requires_grad = True
for param in text_encoder.pooler.parameters():
    param.requires_grad = True

print("已放开 BERT 的最后3层 encoder 和 pooler 进行训练")
hidden_size = 768
