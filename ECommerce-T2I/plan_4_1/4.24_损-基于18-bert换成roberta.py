...
class TextConditionedDDPMUNet(nn.Module):
    def __init__(self, text_emb_size, ddpm_model_path, text_channels=16):
        ...
        self.text_to_channels = nn.Linear(text_emb_size, self.text_channels)
    ...
    def forward(self, x, t, class_labels, text_emb):
        ...
        cond_vec = self.text_to_channels(text_emb)
        cond_map = cond_vec.view(bs, self.text_channels, 1, 1).expand(bs, self.text_channels, h, w)
...
def encode_text_batch(text_list, tokenizer, model, device, grad=False):
    ...
    if hasattr(outputs, "pooler_output") and outputs.pooler_output is not None:
        ...
        raise RuntimeError("text model outputs missing pooler_output/last_hidden_state")
    return text_emb
...
# 加载BERT
roberta_path = "/mnt/d/HuggingFaceModels/models--hfl--chinese-roberta-wwm-ext/"
tokenizer = AutoTokenizer.from_pretrained(roberta_path, use_fast=True)
text_encoder = AutoModel.from_pretrained(roberta_path).to(device)
...
for param in text_encoder.encoder.layer[-1:].parameters():
    param.requires_grad = True
if hasattr(text_encoder, "pooler") and text_encoder.pooler is not None:
    for param in text_encoder.pooler.parameters():
        param.requires_grad = True

print("已放开文本编码器的最后1层 encoder 和 pooler 进行训练")
hidden_size = getattr(getattr(text_encoder, "config", None), "hidden_size", 768)
