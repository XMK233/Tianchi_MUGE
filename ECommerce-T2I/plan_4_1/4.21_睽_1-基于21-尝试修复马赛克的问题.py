...
class TransfusionBackbone(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_ch=1, dim=512, depth=8, heads=8, 
        ...
        # 改进 1：使用卷积 + PixelShuffle 代替纯线性层，消除马赛克效应
        # 原来的 Linear 层是独立预测每个 patch 的像素，导致边界不连续
        # 这里用 3x3 卷积让 patch 之间有信息交流，再用 PixelShuffle 还原
        self.decoder = nn.Sequential(
            nn.Conv2d(dim, patch_size * patch_size * in_ch, kernel_size=3, padding=1),
    ...
    def forward(self, x, t, class_labels, text_emb):
        ...
        # 改进 2：先恢复成 2D 特征图，利用卷积融合邻域信息，再通过 PixelShuffle 上采样
        # [B, N, dim] -> [B, dim, N] -> [B, dim, h_p, w_p]
        x = x.transpose(1, 2).view(B, self.dim, h_p, w_p)
        x = self.decoder(x)
