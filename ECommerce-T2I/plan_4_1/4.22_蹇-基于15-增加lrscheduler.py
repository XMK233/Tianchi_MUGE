...
class ClassConditionedUViT(nn.Module):
    """
    普通的 ViT 版本 ClassConditionedUViT：
    - 去除了 MoE 架构。
    - 使用标准的 ViT 堆叠处理瓶颈特征。
    - 时间步和类别嵌入作为额外的 Token 输入 ViT。
    """
    def __init__(self, num_classes=1, class_emb_size=4, vit_dim=256, vit_heads=4, vit_layers=6, time_emb_size=32, text_emb_size=768):
        super().__init__()
        # 条件嵌入
        ...
        # 条件投影 (Time + Class -> ViT Dim)
        self.cond_proj = nn.Linear(time_emb_size + class_emb_size, vit_dim)

        # ViT Blocks
        self.blocks = nn.ModuleList([ViTBlock(vit_dim, vit_heads) for _ in range(vit_layers)])
        self.norm = nn.LayerNorm(vit_dim)
        self.proj_out = nn.Conv2d(vit_dim, 128, 1)
    ...
    def forward(self, x, t, class_labels, text_emb):
        ...
        cond_token = self.cond_proj(cond).unsqueeze(1) # [bs, 1, vit_dim]
        
        # 拼接条件 Token
        s = torch.cat([cond_token, s], dim=1)
...
net = ClassConditionedUViT(text_emb_size=hidden_size).to(device)
loss_fn = nn.MSELoss()
...
# 增加学习率调度器，当 loss 不再下降时降低学习率
scheduler = lr_scheduler.ReduceLROnPlateau(opt, mode='min', factor=0.5, patience=5)
...
net_ema = ClassConditionedUViT(text_emb_size=hidden_size).to(device)
                ...
                try:
                    ...
                    if 'scheduler_state_dict' in ckpt:
                        try:
                            ...
                            print(f"加载 scheduler 状态失败，使用新的 scheduler: {e}")
