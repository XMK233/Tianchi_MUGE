...
_save_dir = "/mnt/d/forCoding_data/Tianchi_MUGE/plan_4/trained_models"
# 如果保存目录不存在，则创建
...
class SpatialCrossAttention(nn.Module):
    def __init__(self, img_channels, text_emb_size, out_channels, num_heads=4, inner_dim=256):
        super().__init__()
        self.inner_dim = inner_dim
        self.num_heads = num_heads
        
        # Image features -> Query [B, H*W, inner_dim]
        self.to_q = nn.Linear(img_channels, inner_dim)
        
        # Text features -> Key, Value [B, L, inner_dim]
        self.to_k = nn.Linear(text_emb_size, inner_dim)
        self.to_v = nn.Linear(text_emb_size, inner_dim)
        
        self.attn = nn.MultiheadAttention(inner_dim, num_heads, batch_first=True)
        
        self.norm = nn.LayerNorm(inner_dim)
        self.to_out = nn.Linear(inner_dim, out_channels)

    def forward(self, img_feat, text_emb):
        # img_feat: [B, C, H, W]
        # text_emb: [B, L, D] (sequence) or [B, D] (vector)
        
        b, c, h, w = img_feat.shape
        
        # Prepare Query
        # [B, C, H, W] -> [B, H*W, C]
        q_in = img_feat.flatten(2).transpose(1, 2)
        q = self.to_q(q_in) # [B, H*W, inner_dim]
        
        # Prepare Key, Value
        if text_emb.dim() == 2:
            text_emb = text_emb.unsqueeze(1) # [B, 1, D]
            
        k = self.to_k(text_emb) # [B, L, inner_dim]
        v = self.to_v(text_emb) # [B, L, inner_dim]
        
        # Attention
        # attn_out: [B, H*W, inner_dim]
        attn_out, _ = self.attn(q, k, v)
        
        # Norm & Project
        out = self.norm(attn_out)
        out = self.to_out(out) # [B, H*W, out_channels]
...
class TextConditionedDDPMUNet(nn.Module):
    def __init__(self, text_emb_size, ddpm_model_path, text_channels=16):
        super().__init__()
        self.unet = UNet2DModel.from_pretrained(ddpm_model_path)
        self.base_in_ch = self.unet.config.in_channels
        self.text_channels = text_channels
        
        # Replaced Linear with SpatialCrossAttention
        self.fusion_attn = SpatialCrossAttention(
            img_channels=self.base_in_ch,
        ...
        self._extend_unet_input_channels(self.base_in_ch + self.text_channels)

    def _extend_unet_input_channels(self, new_in_ch):
        old_conv = self.unet.conv_in
        old_weight = old_conv.weight
        old_in_ch = old_weight.shape[1]
        if new_in_ch <= old_in_ch:
            self.unet.config.in_channels = new_in_ch
            return
        with torch.no_grad():
            ...
            nn.init.kaiming_normal_(new_weight[:, old_in_ch:])
        new_conv = nn.Conv2d(
    ...
    def forward(self, x, t, class_labels, text_emb):
        bs, c, h, w = x.shape
        if c == 1:
            x_img = x.repeat(1, self.base_in_ch, 1, 1)
        elif c == self.base_in_ch:
            x_img = x
        else:
            x_img = x[:, :self.base_in_ch]
            
        # Use Cross-Attention to generate condition map
        # text_emb is expected to be [B, L, D]
        cond_map = self.fusion_attn(x_img, text_emb)
        
        z = torch.cat([x_img, cond_map], dim=1)
        out = self.unet(z, t).sample
        if out.shape[1] != 1:
...
def encode_text_batch(text_list, tokenizer, model, device, grad=False):
    ...
    # 修改为返回完整序列，以便进行 Attention
    text_emb = hidden.to(torch.float32)
...
net = TextConditionedDDPMUNet(text_emb_size=hidden_size, ddpm_model_path=ddpm_model_path).to(device)
loss_fn = nn.MSELoss()
...
net_ema = TextConditionedDDPMUNet(text_emb_size=hidden_size, ddpm_model_path=ddpm_model_path).to(device)
