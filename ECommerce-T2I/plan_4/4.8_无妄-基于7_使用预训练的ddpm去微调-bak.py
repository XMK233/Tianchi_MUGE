...
class ClassConditionedUViT(nn.Module):
    ...
    其他接口保持不变。
    """
    def __init__(self, num_classes=1, class_emb_size=4, vit_dim=256, vit_heads=4, vit_layers=6, num_experts=8, time_emb_size=32, text_emb_size=768):
        super().__init__()
        # 条件嵌入
...
class TextConditionedDDPMUNet(nn.Module):
    def __init__(self, text_emb_size, ddpm_model_path):
        super().__init__()
        self.unet = UNet2DModel.from_pretrained(ddpm_model_path)
        in_ch = self.unet.config.in_channels
        self.text_to_cond = nn.Linear(text_emb_size, in_ch)

    def forward(self, x, t, class_labels, text_emb):
        bs, _, h, w = x.shape
        in_ch = self.unet.config.in_channels
        if x.shape[1] != in_ch:
            x_in = x.repeat(1, in_ch, 1, 1)
        else:
            x_in = x
        cond = self.text_to_cond(text_emb).view(bs, in_ch, 1, 1).expand(bs, in_ch, h, w)
        z = x_in + cond
        ...
        if isinstance(ckpt, dict):
            if 'model_state_dict' in ckpt:
                try:
                    ...
                    # 显式不加载 scaler 状态，防止旧的缩放因子导致 resume 时梯度爆炸
                    # if 'scaler_state_dict' in ckpt:
                    #     try:
                    #         scaler.load_state_dict(ckpt['scaler_state_dict'])
                    #         print("已加载 scaler 状态")
                    #     except Exception as e:
                    #         print(f"加载 scaler 状态失败: {e}")
                    print("为保证数值稳定性，重置 scaler 状态（不加载旧状态）")
...
for epoch in range(start_epoch, n_epochs):
    ...
    for step, (x, text) in enumerate(tqdm(train_dataloader)):
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            print(f"Warning: Loss is {loss.item()} at step {step}, skipping update & resetting scaler.")
            opt.zero_grad()
            scaler.update(new_scale=128.0) # 重置 scaler
            continue
