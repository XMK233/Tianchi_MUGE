...
def sobel_edges(img):
    ...
    mag = torch.sqrt(ex * ex + ey * ey + 1e-6)
    return mag
...
class DiffusionModelWrapper(nn.Module):
    ...
    def forward(self, latents, text_emb, real_images=None):
        """
        训练时的前向传播：计算噪声预测损失
        latents: [B, 4, 32, 32]
        ...
        loss = loss_mse
        if (self.lambda_tv > 0 or self.lambda_edge > 0) and (self.vae is not None):
            try:
                rec_img = self.vae.decode(noisy_latents)
                rec_img = F.interpolate(rec_img, size=(IMAGE_SIZE, IMAGE_SIZE), mode='bilinear', align_corners=False)
                if self.lambda_tv > 0:
                    loss = loss + self.lambda_tv * tv_loss(rec_img)
                if self.lambda_edge > 0 and real_images is not None:
                    real_img = (real_images + 1) / 2
                    real_img = torch.clamp(real_img, 0, 1)
                    real_img = F.interpolate(real_img, size=(IMAGE_SIZE, IMAGE_SIZE), mode='bilinear', align_corners=False)
                    edge_rec = sobel_edges(rec_img)
                    edge_real = sobel_edges(real_img)
                    loss = loss + self.lambda_edge * F.l1_loss(edge_rec, edge_real)
            except Exception as e:
                pass
        return loss
            ...
            for batch in pbar:
                ...
                try:
                    ...
                    with torch.amp.autocast("cuda"):
                        loss = model(latents, text_emb, images)
