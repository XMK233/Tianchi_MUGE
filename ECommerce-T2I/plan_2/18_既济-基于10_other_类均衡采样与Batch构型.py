...
class ECommerceDataset(Dataset):
    def __init__(self, img_path, text_path, tokenizer, text_encoder, chunk_size=10, start_line=0, label_dict=None):
        ...
        if label_dict is not None and len(self.data) > 0:
            self.calculate_weights(label_dict)
            
    def calculate_weights(self, label_dict):
        targets = []
        ...
        weights = []
        # 避免除以零
        avg_weight = 1.0 / (sum(counts.values()) / len(counts)) if len(counts) > 0 else 1.0
        
        for t in targets:
            if t == "unknown":
                weights.append(avg_weight)
            else:
                weights.append(1.0 / counts[t])
...
def train(tokenizer, text_encoder):
    ...
    try:
        if os.path.exists(label_path):
            ...
            # 尝试使用 tab 分隔符读取 (因为是 tsv 格式的可能性较大)
            df = pd.read_csv(label_path, sep='\t')
            if 'coarse_label' not in df.columns:
                # 尝试逗号分隔符
                df = pd.read_csv(label_path, sep=',')
            
            if 'img_id' in df.columns and 'coarse_label' in df.columns:
                # 建立 img_id -> coarse_label 的映射
                label_dict = pd.Series(df.coarse_label.values, index=df.img_id).to_dict()
                print(f"成功加载 {len(label_dict)} 个标签用于类均衡采样")
            else:
                print("标签文件缺少必要的列 (img_id, coarse_label)")
        else:
    ...
    for chunk_idx in range(latest_chunk, total_chunks):
        ...
        try:
            ...
            if train_dataset.sample_weights is not None:
                # 使用 WeightedRandomSampler 进行类均衡采样
                # replacement=True 允许重复采样，这对均衡很重要
                ...
                print("已启用 WeightedRandomSampler 进行类均衡采样")
            else:
