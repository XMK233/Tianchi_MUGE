'''灵感来源于，之前做的汉服模型，把epoch轮数加到上百之后，效果有所提升。
所以这里我们也接着尝试增加轮数吧。'''
...
class ClassConditionedUViT(nn.Module):
    ...
    def __init__(self, text_emb_size=768, class_emb_size=128, vit_dim=256, vit_heads=4, vit_layers=6, num_experts=4, time_emb_size=32, num_classes=NUM_CLASSES, label_emb_size=64):
        ...
        self.label_gate_proj = nn.Linear(label_emb_size, num_experts)
    ...
    def forward(self, x, t, text_emb, labels):
        ...
        self.label_gate_proj = self.label_gate_proj.to(device)
        
        label_emb_vec = self.label_emb(labels) # [B, label_emb_size]
        label_gate_bias = self.label_gate_proj(label_emb_vec) # [B, num_experts]
        ...
        # 加入标签引导
        gate_logits = gate_logits + label_gate_bias
...
def train(tokenizer, text_encoder):
    ...
    for chunk_idx in range(latest_chunk, total_chunks):
        ...
        try:
            ...
            if loaded_scheduler_state is not None and chunk_idx == latest_chunk:
                try:
                    scheduler.load_state_dict(loaded_scheduler_state)
                except Exception as e:
                    print(f"加载学习率调度器状态失败: {e}")
