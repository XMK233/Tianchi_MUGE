...
class ECommerceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 获取标签
        label_id = self.labels.get(img_id, 0)
        if label_id == -1:
            label_id = 0
...
class ClassConditionedUViT(nn.Module):
    ...
    def forward(self, x, t, text_emb, labels=None):
        ...
        # 标签条件处理
        if labels is None:
            labels = torch.full((bs,), self.num_classes, dtype=torch.long, device=device)
        else:
            labels = labels.to(device)
        c_label = self.label_emb(labels)
...
class DiffusionModelWrapper(nn.Module):
    ...
    def sample(self, text_emb, labels=None, num_inference_steps=50):
        ...
        # 两阶段：前半仅使用标签条件；后半叠加文本条件
        half = len(timesteps) // 2
        stage1_ts = timesteps[:half]
        stage2_ts = timesteps[half:]
        
        # 阶段1：标签条件（文本置零）
        null_text_emb = torch.zeros_like(text_emb)
        for t in stage1_ts:
            t_int = int(t.item())
            noise_pred = self.model(latents, torch.tensor(t_int, dtype=torch.long, device=device), null_text_emb, labels)
...
if __name__ == "__main__":
    ...
    generated_images = generate_images(model, vae, text_encoder, tokenizer, descriptions, labels=val_labels if len(val_labels)>0 else None)
