...
try:
    if os.path.exists(LABEL_CSV_PATH):
        ...
        print(f"类别列表: {unique_labels}")
    else:
        print(f"警告: 标签文件不存在 {LABEL_CSV_PATH}")
...
class ECommerceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 获取标签ID
        coarse_label = IMG_ID_TO_LABEL.get(img_id, None)
        if coarse_label is None:
            label_id = 0 # 默认
        else:
            label_id = LABEL_TO_ID.get(coarse_label, 0)
...
# 4. ClassConditionedUViT类 (修改版：支持Label Embedding)
class ClassConditionedUViT(nn.Module):
    """
    MoE 版本的 ClassConditionedUViT：
    - 支持Label Embedding拼接到条件通道。
    """
    def __init__(self, text_emb_size=768, class_emb_size=128, vit_dim=256, vit_heads=4, vit_layers=6, num_experts=4, time_emb_size=32, num_classes=None, class_emb_size_label=64):
        super().__init__()
        ...
        # 2. 标签嵌入
        self.label_emb = nn.Embedding(num_classes, class_emb_size_label)
        
        self.t_emb = nn.Embedding(1000, time_emb_size)
        
        # 编码器（共享）
        # 输入为4通道latent + class_emb_size (text) + class_emb_size_label (label) 通道条件
        in_ch = 4 + class_emb_size + class_emb_size_label
        ...
        # 门控网络：基于文本嵌入、时间步嵌入、全局特征 和 标签嵌入
        # c_feat 现在包含了 text 和 label 的特征
        gate_in_dim = 128 + class_emb_size + class_emb_size_label + time_emb_size
    ...
    def forward(self, x, t, text_emb, label_ids):
        # 确保所有输入都在同一设备上
        device = x.device
        t = t.to(device)
        ...
        c_feat_text = self.text_emb_proj(text_emb_avg)  # [B, class_emb_size]
        
        # 2. 标签条件处理
        c_feat_label = self.label_emb(label_ids) # [B, class_emb_size_label]
        
        # 3. 拼接条件
        c_feat = torch.cat([c_feat_text, c_feat_label], dim=1) # [B, class_emb_size + class_emb_size_label]
        c_feat = c_feat.to(device)
        
        # 扩展到空间维度
        c2d = c_feat.view(bs, -1, 1, 1).expand(bs, -1, h, w)  # [B, total_cond_dim, H, W]
        c2d = c2d.to(device)
        
        # 将条件拼接到输入
        z = torch.cat([x, c2d], 1)  # [B, 4 + total_cond_dim, H, W]
        z = z.to(device)
        ...
        p3_pool = p3_pool.to(device)
        
        # 门控输入包含 label 信息 (通过 c_feat)
        gate_in = torch.cat([p3_pool, c_feat, t_feat], dim=1)
        gate_in = gate_in.to(device)
...
class DiffusionModelWrapper(nn.Module):
    def __init__(self):
        ...
        self.model = ClassConditionedUViT(vit_dim = IMAGE_SIZE, num_classes=NUM_CLASSES)
    ...
    def forward(self, latents, text_emb, label_ids):
        """
        训练时的前向传播：计算噪声预测损失
        """
        # 1. 采样时间步 t
        ...
        noise_pred = self.model(noisy_latents, timesteps, text_emb, label_ids)
    ...
    @torch.no_grad()
    def sample(self, text_emb, label_ids, num_inference_steps=50):
        """
        生成时的采样过程
        """
        # 确保模型在正确的设备上
        ...
        for t in timesteps:
            ...
            # 预测噪声
            noise_pred = self.model(latents, torch.tensor(t_int, dtype=torch.long, device=device), text_emb, label_ids)
    ...
    for chunk_idx in range(latest_chunk, total_chunks):
        ...
        for epoch in range(start_epoch, NUM_EPOCHS):
            ...
            for batch in pbar:
                ...
                label_ids = batch["label_id"].to(device) # [B]
                ...
                try:
                    ...
                    with torch.amp.autocast("cuda"):
                        loss = model(latents, text_emb, label_ids)
...
# 6. 生成函数
def generate_images(model, vae, text_encoder, tokenizer, descriptions, label_ids):
    print("\n=== 开始生成图像 ===")
    model.eval()
    generated_images = []
    ...
    for i, desc in enumerate(tqdm(descriptions, desc="生成中")):
        ...
        # 获取当前样本的 label_id
        if label_ids is not None:
            curr_label_id = label_ids[i].unsqueeze(0) # [1]
        else:
            curr_label_id = torch.tensor([0], device=device)

        # 采样 (Latent Diffusion)
        try:
            latents = model.sample(text_emb, curr_label_id, num_inference_steps=30) # 减少步数以加快演示
...
if __name__ == "__main__":
    print("电商文本到图像生成 - Diffusion 版 (with Coarse Label Embedding)")
    ...
    for batch in val_loader:
        ...
        val_label_ids.extend(batch["label_id"])
        # if len(descriptions) >= 4: break # 仅测试少量
    
    if len(val_label_ids) > 0:
        val_label_ids = torch.stack(val_label_ids).to(device)
    else:
        val_label_ids = None

    generated_images = generate_images(model, vae, text_encoder, tokenizer, descriptions, val_label_ids)
