...
# FiLM Block Helper Classes
class FiLMGen(nn.Module):
    def __init__(self, label_emb_size, channels):
        super().__init__()
        self.linear = nn.Linear(label_emb_size, channels * 2) # scale, bias

    def forward(self, x, label_emb):
        # label_emb: [B, label_emb_size]
        style = self.linear(label_emb) # [B, channels*2]
...
class DownBlockFiLM(nn.Module):
    def __init__(self, in_ch, out_ch, label_emb_size):
        super().__init__()
        self.c1 = nn.Conv2d(in_ch, out_ch, 3, 1, 1)
        self.gn1 = nn.GroupNorm(8 if out_ch < 128 else 16, out_ch)
        self.film1 = FiLMGen(label_emb_size, out_ch)
        
        self.c2 = nn.Conv2d(out_ch, out_ch, 3, 1, 1)
        self.gn2 = nn.GroupNorm(8 if out_ch < 128 else 16, out_ch)
        self.film2 = FiLMGen(label_emb_size, out_ch)

    def forward(self, x, label_emb):
        x = self.c1(x)
        x = self.gn1(x)
        x = self.film1(x, label_emb)
        x = F.gelu(x)
        
        x = self.c2(x)
        x = self.gn2(x)
        x = self.film2(x, label_emb)
        x = F.gelu(x)
        return x
...
LABEL_CSV_PATH = "/mnt/d/forCoding_code/Tianchi_MUGE/ECommerce-T2I/plan_2/1-利用文本进行分类/classification_results_coarse.csv"
...
try:
    if os.path.exists(LABEL_CSV_PATH):
        label_df = pd.read_csv(LABEL_CSV_PATH, sep='\t')
        # 确保img_id是字符串
        label_df['img_id'] = label_df['img_id'].astype(str)
        IMG_ID_TO_LABEL = dict(zip(label_df['img_id'], label_df['coarse_label']))
        unique_labels = sorted(label_df['coarse_label'].dropna().unique().tolist())
        LABEL_TO_ID = {label: idx for idx, label in enumerate(unique_labels)}
        NUM_CLASSES = len(unique_labels)
        print(f"成功加载标签数据: {len(IMG_ID_TO_LABEL)} 条记录, {NUM_CLASSES} 个类别")
    else:
        print(f"标签文件不存在: {LABEL_CSV_PATH}")
...
class ECommerceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 获取粗粒度标签
        label_str = IMG_ID_TO_LABEL.get(img_id, None)
        if label_str is not None and label_str in LABEL_TO_ID:
...
class ClassConditionedUViT(nn.Module):
    ...
    接口适配目标文件需求。
    """
    def __init__(self, text_emb_size=768, class_emb_size=128, vit_dim=256, vit_heads=4, vit_layers=6, num_experts=4, time_emb_size=32, num_classes=NUM_CLASSES, label_emb_size=64):
        super().__init__()
        # 条件嵌入
        ...
        # 标签嵌入
        self.label_emb = nn.Embedding(num_classes, label_emb_size)
        
        # 编码器（共享）
        # 输入为4通道latent + class_emb_size通道条件
        in_ch = 4 + class_emb_size
        self.down1 = DownBlockFiLM(in_ch, 64, label_emb_size)
        self.pool1 = nn.Conv2d(64, 64, 3, 2, 1)
        self.down2 = DownBlockFiLM(64, 128, label_emb_size)
        self.pool2 = nn.Conv2d(128, 128, 3, 2, 1)
        self.down3 = DownBlockFiLM(128, 128, label_emb_size)
        ...
        # 解码器（共享）
        self.up1 = UpBlockFiLM(128 + 128, 128, label_emb_size)
        self.up2 = UpBlockFiLM(128 + 128, 128, label_emb_size)
        self.up3 = UpBlockFiLM(128 + 64, 64, label_emb_size)
    ...
    def forward(self, x, t, text_emb, labels):
        # 确保所有输入都在同一设备上
        device = x.device
        t = t.to(device)
        ...
        # 获取标签嵌入
        self.label_emb = self.label_emb.to(device)
        label_emb_vec = self.label_emb(labels) # [B, label_emb_size]
        ...
        d1 = self.down1(z, label_emb_vec)
        p1 = self.pool1(d1)
        d2 = self.down2(p1, label_emb_vec)
        p2 = self.pool2(d2)
        d3 = self.down3(p2, label_emb_vec)
        p3 = self.pool3(d3)
...
class DiffusionModelWrapper(nn.Module):
    ...
    def forward(self, latents, text_emb, labels):
        """
        训练时的前向传播：计算噪声预测损失
        latents: [B, 4, 32, 32]
    ...
    @torch.no_grad()
    def sample(self, text_emb, labels=None, num_inference_steps=50):
        """
        生成时的采样过程
        """
        # 确保模型在正确的设备上
        ...
        for t in timesteps:
            ...
            # 预测噪声
            noise_pred = self.model(latents, torch.tensor(t_int, dtype=torch.long, device=device), text_emb, labels)
...
if __name__ == "__main__":
    ...
    for batch in val_loader:
        ...
        val_labels.extend(batch["label_id"])
        # if len(descriptions) >= 4: break # 仅测试少量
    
    # 转换 val_labels 为 tensor
