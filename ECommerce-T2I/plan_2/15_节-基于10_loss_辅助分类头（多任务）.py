...
class ClassConditionedUViT(nn.Module):
    ...
    def forward(self, x, t, text_emb):
        ...
        p3_pool = F.adaptive_avg_pool2d(p3, 1).view(bs, 128)
        ...
        cls_logits = self.cls_head(p3_pool)
...
class DiffusionModelWrapper(nn.Module):
    ...
    def forward(self, latents, text_emb, labels=None, lambda_cls=0.2):
        """
        训练时的前向传播：计算噪声预测损失
        latents: [B, 4, 32, 32]
        ...
        noise_pred, cls_logits = self.model(noisy_latents, timesteps, text_emb)
        
        # 5. 计算损失 (MSE Loss between actual noise and predicted noise)
        loss_mse = F.mse_loss(noise_pred, noise)
        if labels is not None:
            cls_loss = F.cross_entropy(cls_logits, labels)
            loss = loss_mse + lambda_cls * cls_loss
        else:
            loss = loss_mse
        return loss
