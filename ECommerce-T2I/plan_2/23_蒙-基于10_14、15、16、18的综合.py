...
# 改进3: 读取 coarse_label 映射并统计类别数量（类均衡采样与Batch构型）
COARSE_LABEL_CSV = "/mnt/d/forCoding_code/Tianchi_MUGE/ECommerce-T2I/plan_2/1-利用文本进行分类/classification_results_coarse.csv"
IMGID_TO_LABEL = {}
LABEL_TO_ID = {}
NUM_CLASSES = 0
try:
    if os.path.exists(COARSE_LABEL_CSV):
        with open(COARSE_LABEL_CSV, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            # 兼容不同列名：尝试常见字段
            possible_img_cols = ["img_id", "image_id", "id"]
            possible_label_cols = ["coarse_label", "coarse_label_id", "label", "category"]
            header = reader.fieldnames or []
            img_col = next((c for c in possible_img_cols if c in header), None)
            label_col = next((c for c in possible_label_cols if c in header), None)
            tmp_labels = []
            rows = list(reader)
            if img_col is None or label_col is None:
                # 退化处理：当 DictReader 无法识别时，按手动 split
                f.seek(0)
                ...
                for line in lines[1:]:
                    ...
                    if len(parts) >= 2:
                        img_id = parts[0]
                        label_name = parts[1]
                        tmp_labels.append(label_name)
                        IMGID_TO_LABEL[img_id] = label_name
            else:
                for row in rows:
                    ...
                    IMGID_TO_LABEL[img_id] = label_name
            # 构造 label→id
            unique_labels = sorted(set(tmp_labels))
            LABEL_TO_ID = {name: idx for idx, name in enumerate(unique_labels)}
            NUM_CLASSES = len(unique_labels)
            print(f"成功加载标签数据: {len(IMGID_TO_LABEL)} 条记录, {NUM_CLASSES} 个类别")
    else:
        print(f"未找到 coarse_label 文件: {COARSE_LABEL_CSV}，将使用单一类别")
        LABEL_TO_ID = {"unknown": 0}
        NUM_CLASSES = 1
except Exception as e:
    print(f"读取 coarse_label 文件失败: {e}，将使用单一类别")
    LABEL_TO_ID = {"unknown": 0}
    NUM_CLASSES = 1
...
class ECommerceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 改进3: 返回 coarse_label_id 以支持类均衡采样与损失计算
        label_name = IMGID_TO_LABEL.get(str(img_id), next(iter(LABEL_TO_ID.keys())))
        coarse_label_id = LABEL_TO_ID.get(label_name, 0)
        return {
            "img_id": img_id,
            "image": img_tensor, # [-1, 1]
            "text_emb": text_emb.squeeze(0),  # [MAX_TEXT_LENGTH, 768]
            "description": description,
            "coarse_label_id": coarse_label_id
        }
...
class ClassConditionedUViT(nn.Module):
    ...
    def __init__(self, text_emb_size=768, class_emb_size=128, vit_dim=256, vit_heads=4, vit_layers=6, num_experts=4, time_emb_size=32):
        ...
        # 改进1: 引入 coarse_label 的嵌入用于 MoE 路由主导
        self.label_emb = nn.Embedding(NUM_CLASSES, class_emb_size)
    ...
    def forward(self, x, t, text_emb, coarse_label_id):
        # 确保所有输入都在同一设备上
        device = x.device
        t = t.to(device)
        text_emb = text_emb.to(device)
        coarse_label_id = coarse_label_id.to(device)
        ...
        # 改进1: MoE 路由改为标签主导（concat label_emb + p3_pool + t_emb）
        label_feat = self.label_emb(coarse_label_id)  # [bs, class_emb_size]
        gate_in = torch.cat([p3_pool, label_feat, t_feat], dim=1)
        ...
        # 改进2: 输出辅助分类 logits 供多任务损失
        cls_logits = self.cls_head(p3_pool)  # [bs, NUM_CLASSES]
        return output, cls_logits, p3_pool
...
class DiffusionModelWrapper(nn.Module):
    ...
    def forward(self, latents, text_emb, coarse_label_id):
        ...
        noise_pred, cls_logits, p3_pool = self.model(noisy_latents, timesteps, text_emb, coarse_label_id)
        
        # 5. 计算损失 (MSE Loss between actual noise and predicted noise)
        mse_loss = F.mse_loss(noise_pred, noise)
        # 改进2: 辅助分类损失
        cls_loss = F.cross_entropy(cls_logits, coarse_label_id)
        # 改进2: InfoNCE 对比损失（样本特征 vs 类原型）
        feats = F.normalize(p3_pool, dim=1)  # [bs, 128]
        protos = F.normalize(self.prototypes, dim=1)  # [C, 128]
        logits = (feats @ protos.t()) / self.temperature  # [bs, C]
        contrast_loss = F.cross_entropy(logits, coarse_label_id)
        loss = mse_loss + self.lambda_cls * cls_loss + self.lambda_contrast * contrast_loss
    ...
    def sample(self, text_emb, num_inference_steps=50):
        ...
        # 3. 确保文本嵌入在正确的设备上
        text_emb = text_emb.to(device)
        # 改进1兼容: 采样阶段默认使用零类标签（无CFG情况下）
        coarse_label_id = torch.zeros(bsz, dtype=torch.long, device=device)
        ...
        for t in timesteps:
            ...
            # 预测噪声
            noise_pred, _, _ = self.model(latents, torch.tensor(t_int, dtype=torch.long, device=device), text_emb, coarse_label_id)
...
def train(tokenizer, text_encoder):
    ...
    for chunk_idx in range(latest_chunk, total_chunks):
        ...
        try:
            ...
            # 改进3: 类均衡采样（WeightedRandomSampler）
            labels = [train_dataset[i]["coarse_label_id"] for i in range(len(train_dataset))]
            class_counts = np.bincount(labels, minlength=max(1, NUM_CLASSES))
        ...
        for epoch in range(start_epoch, NUM_EPOCHS):
            ...
            for batch in pbar:
                ...
                coarse_label_id = batch["coarse_label_id"].to(device) # [B]
                ...
                try:
                    ...
                    with torch.amp.autocast("cuda"):
                        loss = model(latents, text_emb, coarse_label_id)
