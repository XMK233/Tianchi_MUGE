...
class ECommerceDataset(Dataset):
    ...
    def load_labels(self):
        label_path = "/mnt/d/forCoding_code/Tianchi_MUGE/ECommerce-T2I/plan_2/1-利用文本进行分类/classification_results_coarse.csv"
        if os.path.exists(label_path):
            try:
                ...
                if 'coarse_label' in df.columns:
                    unique_labels = sorted(df['coarse_label'].unique().tolist())
                    self.label_map = {l: i for i, l in enumerate(unique_labels)}
                    # print(f"标签映射: {self.label_map}")
                    
                    for _, row in df.iterrows():
                        self.labels[row['img_id']] = self.label_map.get(row['coarse_label'], -1)
    ...
    def __getitem__(self, idx):
        ...
        # 获取标签
        label_id = self.labels.get(img_id, 0) # 默认为0
        if label_id == -1: label_id = 0
...
class ClassConditionedUViT(nn.Module):
    ...
    接口适配目标文件需求。
    """
    def __init__(self, text_emb_size=768, class_emb_size=128, vit_dim=256, vit_heads=4, vit_layers=6, num_experts=4, time_emb_size=32, num_classes=20):
        super().__init__()
        # 条件嵌入
        # 这里使用线性层将文本嵌入转换为类别条件嵌入
        self.text_emb_proj = nn.Linear(text_emb_size, class_emb_size)
        # 增加标签嵌入层，多一个用于无条件(null label)
        self.label_emb = nn.Embedding(num_classes + 1, class_emb_size)
    ...
    def forward(self, x, t, text_emb, labels=None):
        # 确保所有输入都在同一设备上
        device = x.device
        t = t.to(device)
        ...
        c_text = self.text_emb_proj(text_emb_avg)  # [B, class_emb_size]
        
        # 标签条件处理
        if labels is None:
            labels = torch.full((bs,), self.num_classes, device=device, dtype=torch.long)
        else:
            labels = labels.to(device)
            
        c_label = self.label_emb(labels) # [B, class_emb_size]
        
        # 融合条件 (简单相加)
        c_feat = c_text + c_label
...
class DiffusionModelWrapper(nn.Module):
    ...
    def forward(self, latents, text_emb, labels=None):
        """
        训练时的前向传播：计算噪声预测损失
        latents: [B, 4, 32, 32]
        ...
        # Classifier-Free Guidance 训练时的随机 Drop
        # 10% 概率 Drop Text (使用零向量)
        # 10% 概率 Drop Label (使用 num_classes)
        # 10% 概率 Drop Both (Unconditional)
        # 70% Keep Both
        ...
        rand = torch.rand(bsz, device=device)
        
        # 处理 Text Drop
        # Drop Both (0-0.1) or Drop Text (0.1-0.2) -> Drop if < 0.2
        drop_text_mask = rand < 0.2
        if drop_text_mask.any():
            # 将被 drop 的 text_emb 置零
            text_emb = text_emb.clone()
            text_emb[drop_text_mask] = 0
            
        # 处理 Label Drop
        # Drop Both (0-0.1) or Drop Label (0.2-0.3) -> Drop if < 0.1 or (0.2 <= r < 0.3)
        drop_label_mask = (rand < 0.1) | ((rand >= 0.2) & (rand < 0.3))
        if drop_label_mask.any():
            labels = labels.clone()
            labels[drop_label_mask] = self.model.num_classes # Null label
        ...
        noise_pred = self.model(noisy_latents, timesteps, text_emb, labels)
    ...
    @torch.no_grad()
    def sample(self, text_emb, labels=None, num_inference_steps=50, text_guidance_scale=3.0, label_guidance_scale=3.0):
        """
        生成时的采样过程，支持文本和标签的双重引导
        text_emb: [B, SeqLen, 768]
        labels: [B]
        """
        # 确保模型在正确的设备上
        ...
        # 构造 Unconditional 和 Conditional 的输入
        # 我们需要计算三个部分:
        # 1. Unconditional (null text, null label)
        # 2. Text Conditional (text, null label)
        # 3. Label Conditional (null text, label)
        # 最终预测: eps = eps_uncond + s_t * (eps_text - eps_uncond) + s_l * (eps_label - eps_uncond)
        
        # 构造 batch 输入: [Uncond, Text, Label]
        # Text Embeddings
        null_text_emb = torch.zeros_like(text_emb)
        text_emb_in = torch.cat([null_text_emb, text_emb, null_text_emb], dim=0)
        
        # Labels
        null_labels = torch.full((bsz,), self.model.num_classes, dtype=torch.long, device=device)
        labels_in = torch.cat([null_labels, null_labels, labels], dim=0)
        
        # 5. 逐步去噪
        for t in timesteps:
            t_int = int(t.item())
            
            # 扩展 latents 和 timesteps 以匹配 batch size * 3
            latents_in = torch.cat([latents] * 3, dim=0)
            t_in = torch.tensor([t_int], device=device).repeat(bsz * 3)
            
            # 预测噪声
            noise_pred_chunked = self.model(latents_in, t_in, text_emb_in, labels_in)
            
            # 拆分预测结果
            noise_pred_uncond, noise_pred_text, noise_pred_label = noise_pred_chunked.chunk(3, dim=0)
            
            # 组合噪声预测
            noise_pred = noise_pred_uncond + \
                         text_guidance_scale * (noise_pred_text - noise_pred_uncond) + \
                         label_guidance_scale * (noise_pred_label - noise_pred_uncond)
            ...
            for batch in pbar:
                ...
                try:
                    ...
                    with torch.amp.autocast("cuda"):
                        loss = model(latents, text_emb, labels)
...
# 6. 生成函数
def generate_images(model, vae, text_encoder, tokenizer, descriptions, labels=None):
    print("\n=== 开始生成图像 ===")
    model.eval()
    generated_images = []
    
    for i, desc in enumerate(tqdm(descriptions, desc="生成中")):
        # 编码文本
        ...
        if labels is not None and i < len(labels):
            current_label = labels[i].unsqueeze(0).to(device)
            
        # 采样 (Latent Diffusion)
        try:
            latents = model.sample(text_emb, labels=current_label, num_inference_steps=30) # 减少步数以加快演示
...
if __name__ == "__main__":
    ...
    for batch in val_loader:
        ...
        val_labels.extend([lbl for lbl in batch["label"]])
        # if len(descriptions) >= 4: break # 仅测试少量
    
    generated_images = generate_images(model, vae, text_encoder, tokenizer, descriptions, labels=val_labels)
