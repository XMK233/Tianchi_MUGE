...
class ClassConditionedUViT(nn.Module):
    ...
    def __init__(self, text_emb_size=768, class_emb_size=128, vit_dim=256, vit_heads=4, vit_layers=6, num_experts=4, time_emb_size=32):
        ...
        # 编码器（共享）
        # 输入为3通道图像 + class_emb_size通道条件
        in_ch = 3 + class_emb_size
...
class DiffusionModelWrapper(nn.Module):
    ...
    def forward(self, images, text_emb):
        ...
        noise_pred = self.model(noisy_images, timesteps, text_emb)
    ...
    def sample(self, text_emb, num_inference_steps=50):
        ...
        images = torch.randn(bsz, 3, IMAGE_SIZE, IMAGE_SIZE, device=device)
        ...
        for t in timesteps:
            ...
            noise_pred = self.model(images, torch.tensor(t_int, dtype=torch.long, device=device), text_emb)
            
            noise_pred = noise_pred.to(device)
            
            images = images.to(device)
            
            scheduler_output = self.scheduler.step(noise_pred, t_int, images)
            images = scheduler_output.prev_sample.to(device)
...
def generate_images(model, text_encoder, tokenizer, descriptions):
    ...
    for desc in tqdm(descriptions, desc="生成中"):
        ...
        try:
            img = model.sample(text_emb, num_inference_steps=30)
