...
try:
    if os.path.exists(LABEL_CSV_PATH):
        ...
        NUM_CLASSES = len(unique_labels) if len(unique_labels) > 0 else 1
    else:
...
class ECommerceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_str = IMG_ID_TO_LABEL.get(str(img_id), None)
        ...
        return {
            ...
            "coarse_label_id": torch.tensor(label_id, dtype=torch.long)
        }
...
class DiffusionModelWrapper(nn.Module):
    def __init__(self):
        ...
        self.prototypes = nn.Embedding(NUM_CLASSES, 128)
    ...
    def forward(self, latents, text_emb, labels=None, lambda_contrast=0.2):
        """
        训练时的前向传播：计算噪声预测损失
        latents: [B, 4, 32, 32]
        ...
        noise_pred, p3_pool = self.model(noisy_latents, timesteps, text_emb)
        
        # 5. 计算损失 (MSE Loss between actual noise and predicted noise)
        loss_mse = F.mse_loss(noise_pred, noise)
        if labels is not None:
            feat = F.normalize(p3_pool, dim=1)
            proto = F.normalize(self.prototypes.weight, dim=1)
            logits = torch.matmul(feat, proto.t()) / self.temperature
            info_loss = F.cross_entropy(logits, labels)
            loss = loss_mse + lambda_contrast * info_loss
        else:
            loss = loss_mse
        return loss
    ...
    def sample(self, text_emb, num_inference_steps=50):
        ...
        for t in timesteps:
            ...
            # 预测噪声
            noise_pred, _ = self.model(latents, torch.tensor(t_int, dtype=torch.long, device=device), text_emb)
    ...
    for chunk_idx in range(latest_chunk, total_chunks):
        ...
        for epoch in range(start_epoch, NUM_EPOCHS):
            ...
            for batch in pbar:
                ...
                labels = batch["coarse_label_id"].to(device)
