...
parser.add_argument(
    ...
    type=float,
    default=0.05,
    help="用于 margin 类损失的边距参数（AM-Softmax/Triplet）"
)
parser.add_argument(
    "--gamma",
    type=float,
    default=2.0,
    help="Focal 对比损失的聚焦参数 gamma"
)
parser.add_argument(
    "--learn_temp",
    action="store_true",
    help="是否将温度作为可学习参数（CLIP 风格 logit_scale）"
)
parser.add_argument(
    "--lr_temp",
    type=float,
    default=1e-3,
    help="温度参数的学习率（仅在 --learn_temp 时有效）"
)
...
def asymmetric_nce_loss(text_feats: torch.Tensor, image_feats: torch.Tensor, temp: float) -> torch.Tensor:
    logits = torch.mm(text_feats, image_feats.t()) / temp
    labels = torch.arange(logits.size(0), device=logits.device)
    loss_t = torch.nn.functional.cross_entropy(logits, labels)
    return loss_t

def triplet_margin_loss(text_feats: torch.Tensor, image_feats: torch.Tensor, margin: float) -> torch.Tensor:
    sims = torch.mm(text_feats, image_feats.t())
    B = sims.size(0)
    if B <= 1:
    ...
    neg_t_vals = sims.masked_fill(~mask, float('-inf')).max(dim=1).values
    loss_t = torch.nn.functional.relu(margin + neg_t_vals - pos).mean()
    sims_i = sims.t()
    pos_i = sims_i[diag, diag]
    mask_i = torch.ones_like(sims_i, dtype=torch.bool)
    mask_i[diag, diag] = False
    neg_i_vals = sims_i.masked_fill(~mask_i, float('-inf')).max(dim=1).values
    loss_i = torch.nn.functional.relu(margin + neg_i_vals - pos_i).mean()
    return 0.5 * (loss_t + loss_i)

def am_softmax_nce_loss(text_feats: torch.Tensor, image_feats: torch.Tensor, temp: float, margin: float) -> torch.Tensor:
    sims = torch.mm(text_feats, image_feats.t())
    B = sims.size(0)
    diag = torch.arange(B, device=sims.device)
    sims = sims.clone()
    sims[diag, diag] = sims[diag, diag] - margin
    logits = sims / temp
    labels = diag
    loss_t = torch.nn.functional.cross_entropy(logits, labels)
    loss_i = torch.nn.functional.cross_entropy(logits.t(), labels)
...
def focal_nce_loss(text_feats: torch.Tensor, image_feats: torch.Tensor, temp: float, gamma: float) -> torch.Tensor:
    logits = torch.mm(text_feats, image_feats.t()) / temp
    B = logits.size(0)
    labels = torch.arange(B, device=logits.device)
    ce_t = torch.nn.functional.cross_entropy(logits, labels, reduction='none')
    pt_t = torch.softmax(logits, dim=1)[labels, labels]
    weight_t = (1.0 - pt_t).clamp(min=0.0) ** gamma
    loss_t = (weight_t * ce_t).mean()
    logits_i = logits.t()
    ce_i = torch.nn.functional.cross_entropy(logits_i, labels, reduction='none')
    pt_i = torch.softmax(logits_i, dim=1)[labels, labels]
    weight_i = (1.0 - pt_i).clamp(min=0.0) ** gamma
    loss_i = (weight_i * ce_i).mean()
    return 0.5 * (loss_t + loss_i)

def info_loob_loss(text_feats: torch.Tensor, image_feats: torch.Tensor, temp: float) -> torch.Tensor:
    logits = torch.mm(text_feats, image_feats.t()) / temp
    B = logits.size(0)
    diag = torch.arange(B, device=logits.device)
    mask = torch.ones_like(logits, dtype=torch.bool)
    mask[diag, diag] = False
    denom_t = torch.logsumexp(logits.masked_fill(~mask, float('-inf')), dim=1)
    numer_t = logits[diag, diag]
    loss_t = -(numer_t - denom_t).mean()
    logits_i = logits.t()
    denom_i = torch.logsumexp(logits_i.masked_fill(~mask.t(), float('-inf')), dim=1)
    numer_i = logits_i[diag, diag]
    loss_i = -(numer_i - denom_i).mean()
    return 0.5 * (loss_t + loss_i)
...
learn_temp = bool(args.learn_temp)
lr_temp = float(args.lr_temp)

# 可学习温度（logit_scale），参考 CLIP 将 1/temperature 作为可学习参数
logit_scale = None
if learn_temp:
    logit_scale = torch.nn.Parameter(torch.tensor(1.0/temperature, dtype=torch.float32, device=device))
...
if learn_temp and logit_scale is not None:
        ...
        for s in range(0, len(pairs), step_bs):
            ...
            if use_amp and device.type == 'cuda':
                with autocast(enabled=True):
                    ...
                    current_temp = (1.0 / logit_scale.clamp(1e-3, 100.0)) if (learn_temp and logit_scale is not None) else temperature
                    temp_value = current_temp.item() if isinstance(current_temp, torch.Tensor) else float(current_temp)
                    if loss_type == 'symmetric_nce':
                        loss = info_nce_loss(t_proj, i_proj, temp=temp_value)
                    elif loss_type == 'asymmetric_nce':
                        loss = asymmetric_nce_loss(t_proj, i_proj, temp=temp_value)
                    elif loss_type == 'triplet_margin':
                        loss = triplet_margin_loss(t_proj, i_proj, margin=margin)
                    elif loss_type == 'am_softmax_nce':
                        loss = am_softmax_nce_loss(t_proj, i_proj, temp=temp_value, margin=margin)
                    elif loss_type == 'focal_nce':
                        loss = focal_nce_loss(t_proj, i_proj, temp=temp_value, gamma=gamma)
                    elif loss_type == 'info_loob':
                        loss = info_loob_loss(t_proj, i_proj, temp=temp_value)
                    else:
                        loss = info_nce_loss(t_proj, i_proj, temp=temp_value)
                scaler.scale(loss).backward()
                scaler.unscale_(optim)
                torch.nn.utils.clip_grad_norm_(
                    list(model.fusion.text_projector.parameters()) + list(model.fusion.image_projector.parameters()) + (([logit_scale] if (learn_temp and logit_scale is not None) else [])),
                    max_norm=5.0
                )
                scaler.step(optim)
                scaler.update()
