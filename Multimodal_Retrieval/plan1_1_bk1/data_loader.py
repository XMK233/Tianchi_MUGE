...
class DataLoader:
    ...
    def load_images_batch(self, split='train', max_samples=None, batch_size=None, max_batches=None):
        """
        批量加载图片数据，控制内存使用
        ...
        Args:
            ...
            batch_size: 每个批次的图片数量，默认使用初始化的 batch_size
            max_batches: 最多加载的批次数，用于限制训练总量以便调试
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                for _ in f:
                    total_lines += 1
            # print(split, "总共这么多行数据", total_lines)
            
            if max_samples:
                total_lines = min(total_lines, max_samples)
            
            with open(file_path, 'r', encoding='utf-8') as f:
                for line in tqdm(f, desc=f"实际加载{split}图片数据", total=total_lines):
                    try:
            ...
            # 处理最后一个批次（如果未达到最大批次数并且有残留）
            if batch and (max_batches is None or yielded_batches < max_batches):
                yield batch
            
        except Exception as e:
            logger.error(f"批量加载图片数据失败: {e}")
    
    def create_img_id_to_image_dict(self, split='train', max_samples=None, batch_size=None, max_batches=None):
        """
        创建图片ID到图像的映射字典
        ...
        for batch in self.load_images_batch(split=split, max_samples=max_samples, batch_size=batch_size, max_batches=max_batches):
            for item in batch:
