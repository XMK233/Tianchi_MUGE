...
class FeatureFusion:
    ...
    def __init__(self, fusion_method='projection', projection_dim=512, device=None, hidden_dim=1024, dropout=0.1, text_in_dim=768, image_in_dim=2048):
        ...
        if fusion_method == 'projection':
            ...
            # 共享投影头方案：
            # 1) 模态适配：将各自输入维度映射到共同的 projection_dim
            # 2) 共享投影头：对两种模态使用同一套权重进行最终投影
            self.pre_text = torch.nn.Linear(text_in_dim, projection_dim).to(self.device)
            self.pre_image = torch.nn.Linear(image_in_dim, projection_dim).to(self.device)
            self.shared_projector = torch.nn.Linear(projection_dim, projection_dim).to(self.device)
    ...
    def fuse_text_features(self, text_features: torch.Tensor) -> torch.Tensor:
        # 后融合：模态适配后，经过共享投影头；早融合：直接返回原始文本特征
        if self.fusion_method == 'projection':
            return self.shared_projector(self.pre_text(text_features))
        return text_features
    def fuse_image_features(self, image_features: torch.Tensor) -> torch.Tensor:
        # 后融合：模态适配后，经过共享投影头；早融合：直接返回原始图像特征
        if self.fusion_method == 'projection':
            return self.shared_projector(self.pre_image(image_features))
...
class CrossModalRetrievalModel:
    ...
    def extract_and_fuse_text_features(self, texts: List[str]) -> torch.Tensor:
        ...
        if self.fusion.fusion_method == 'projection':
            # 共享投影头评估模式
            self.fusion.shared_projector.eval()
...
def build_optimizer(model: CrossModalRetrievalModel, text_extractor: TextFeatureExtractor, image_extractor: ImageFeatureExtractor,
    ...
    if model.fusion.fusion_method == 'early':
        ...
        # 共享投影头 + 两个轻量适配层
        proj_params = list(model.fusion.shared_projector.parameters())
        adapter_params = []
        if hasattr(model.fusion, 'pre_text'): adapter_params += list(model.fusion.pre_text.parameters())
        if hasattr(model.fusion, 'pre_image'): adapter_params += list(model.fusion.pre_image.parameters())
        params.append({
            'params': proj_params + adapter_params,
            'lr': lr_proj,
            'weight_decay': weight_decay
        })
    text_top_params = []
...
def build_llrd_optimizer(model: CrossModalRetrievalModel, text_extractor: TextFeatureExtractor, image_extractor: ImageFeatureExtractor,
    ...
    params = []
    # 投影层分组：后融合为共享投影头（含适配层）；早融合为统一 pair_projector
    if model.fusion.fusion_method == 'early':
        params.append({
            'params': list(model.fusion.pair_projector.parameters()),
            'lr': lr_proj,
        ...
        proj_params = list(model.fusion.shared_projector.parameters()) if hasattr(model.fusion, 'shared_projector') else []
        adapter_params = []
        if hasattr(model.fusion, 'pre_text'): adapter_params += list(model.fusion.pre_text.parameters())
        if hasattr(model.fusion, 'pre_image'): adapter_params += list(model.fusion.pre_image.parameters())
        params.append({
...
def train_one_batch(pairs: List[Tuple[str, Image.Image, str]], epochs: int, step_bs: int):
    # 共享投影头训练模式
    if hasattr(model.fusion, 'shared_projector'): model.fusion.shared_projector.train()
    if hasattr(model.fusion, 'pre_text'): model.fusion.pre_text.train()
    if hasattr(model.fusion, 'pre_image'): model.fusion.pre_image.train()
    ...
    for e in range(epochs):
        ...
        for s in range(0, len(pairs), step_bs):
            ...
            if use_amp and device.type == 'cuda':
                ...
                clip_params = []
                if hasattr(model.fusion, 'shared_projector'): clip_params += list(model.fusion.shared_projector.parameters())
                if hasattr(model.fusion, 'pre_text'): clip_params += list(model.fusion.pre_text.parameters())
                if hasattr(model.fusion, 'pre_image'): clip_params += list(model.fusion.pre_image.parameters())
                if clip_params:
...
def save_unfreeze_checkpoint(model: CrossModalRetrievalModel, text_extractor: TextFeatureExtractor, image_extractor: ImageFeatureExtractor,
    ...
    # 保存共享投影头/适配层（后融合）或 pair_projector（早融合）
    if getattr(model.fusion, 'fusion_method', 'projection') == 'projection':
        if hasattr(model.fusion, 'shared_projector'):
            ckpt['fusion']['shared_projector'] = model.fusion.shared_projector.state_dict()
        if hasattr(model.fusion, 'pre_text'):
            ckpt['fusion']['pre_text'] = model.fusion.pre_text.state_dict()
        if hasattr(model.fusion, 'pre_image'):
            ckpt['fusion']['pre_image'] = model.fusion.pre_image.state_dict()
    else:
        if hasattr(model.fusion, 'pair_projector'):
            ckpt['fusion']['pair_projector'] = model.fusion.pair_projector.state_dict()
    enc = text_extractor.model.encoder
    total_layers = len(enc.layer)
    start_idx = max(0, total_layers - last_n_layers)
...
def load_unfreeze_checkpoint(model: CrossModalRetrievalModel, text_extractor: TextFeatureExtractor, image_extractor: ImageFeatureExtractor,
    ...
    if getattr(model.fusion, 'fusion_method', 'projection') == 'projection':
        # 优先加载共享投影头与适配层；若是旧版检查点，尝试兼容 text/image_projector -> pre_* 映射
        if 'shared_projector' in ckpt.get('fusion', {}):
            if hasattr(model.fusion, 'shared_projector'):
                model.fusion.shared_projector.load_state_dict(ckpt['fusion']['shared_projector'])
        if 'pre_text' in ckpt.get('fusion', {}):
            if hasattr(model.fusion, 'pre_text'):
                model.fusion.pre_text.load_state_dict(ckpt['fusion']['pre_text'])
        if 'pre_image' in ckpt.get('fusion', {}):
            if hasattr(model.fusion, 'pre_image'):
                model.fusion.pre_image.load_state_dict(ckpt['fusion']['pre_image'])
        # 兼容旧 ckpt：若存在 text_projector/image_projector，则将其权重加载到对应适配层（仅当形状匹配）
        if 'text_projector' in ckpt.get('fusion', {}) and hasattr(model.fusion, 'pre_text'):
            try:
                model.fusion.pre_text.load_state_dict(ckpt['fusion']['text_projector'], strict=False)
            except Exception as _e:
                pass
        if 'image_projector' in ckpt.get('fusion', {}) and hasattr(model.fusion, 'pre_image'):
            try:
                model.fusion.pre_image.load_state_dict(ckpt['fusion']['image_projector'], strict=False)
            except Exception as _e:
                pass
    else:
        if 'pair_projector' in ckpt.get('fusion', {}) and hasattr(model.fusion, 'pair_projector'):
            model.fusion.pair_projector.load_state_dict(ckpt['fusion']['pair_projector'])
