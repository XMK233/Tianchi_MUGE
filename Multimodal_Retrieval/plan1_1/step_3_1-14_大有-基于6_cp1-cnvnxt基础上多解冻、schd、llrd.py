...
def unfreeze_image_top_block(image_extractor: ImageFeatureExtractor, last_n_img_stages: int = 1, unfreeze_layer4: bool = True):
    for p in image_extractor.model.parameters():
        p.requires_grad = False
    # ConvNeXt: 解冻最后 N 个 stages
    if hasattr(image_extractor.model, 'stages'):
        total_stages = len(image_extractor.model.stages)
        start_idx = max(0, total_stages - max(1, last_n_img_stages))
        for i in range(start_idx, total_stages):
            for p in image_extractor.model.stages[i].parameters():
                p.requires_grad = True
            image_extractor.model.stages[i].train()
    # ResNet: 仅解冻 layer4（保持与现有逻辑一致）
...
def build_llrd_optimizer(model: CrossModalRetrievalModel, text_extractor: TextFeatureExtractor, image_extractor: ImageFeatureExtractor,
                         ...
                         last_n_layers: int = 2, last_n_img_stages: int = 1, weight_decay: float = 1e-4):
    params = []
    # 投影层（文本/图像）
    params.append({
        'params': list(model.fusion.text_projector.parameters()) + list(model.fusion.image_projector.parameters()),
    ...
    # 图像侧 LLRD：ConvNeXt 的最后 N 个 stages 逐层衰减；ResNet 保持 layer4 单组
    if hasattr(image_extractor.model, 'stages'):
        total_stages = len(image_extractor.model.stages)
        start_idx = max(0, total_stages - max(1, last_n_img_stages))
        order = 0
        for i in range(total_stages - 1, start_idx - 1, -1):
            group_lr = lr_img_top * (decay ** order)
            params.append({
                'params': [p for p in image_extractor.model.stages[i].parameters() if p.requires_grad],
                'lr': group_lr,
                'weight_decay': 0.0
            })
            order += 1
...
# 微调与调度参数
last_n_layers = 8  # 文本侧顶层解冻层数
last_n_img_stages = 2  # 图像侧解冻的 ConvNeXt 顶层 stages 数
...
unfreeze_image_top_block(image_extractor, last_n_img_stages=last_n_img_stages, unfreeze_layer4=True)
...
optim = build_llrd_optimizer(model, text_extractor, image_extractor,
                             ...
                             last_n_layers=last_n_layers, last_n_img_stages=last_n_img_stages, weight_decay=1e-4)
...
def train_one_batch(pairs: List[Tuple[str, Image.Image, str]], epochs: int, step_bs: int):
    ...
    # 确保已解冻的文本顶层都处于 train 状态
    enc = text_extractor.model.encoder
    total_layers = len(enc.layer)
    start_idx = max(0, total_layers - max(1, last_n_layers))
    ...
    if hasattr(image_extractor.model, 'stages'):
        total_stages = len(image_extractor.model.stages)
        start_idx_img = max(0, total_stages - max(1, last_n_img_stages))
        for i in range(start_idx_img, total_stages):
            image_extractor.model.stages[i].train()
    elif hasattr(image_extractor.model, 'layer4'):
        image_extractor.model.layer4.train()
...
def save_unfreeze_checkpoint(model: CrossModalRetrievalModel, text_extractor: TextFeatureExtractor, image_extractor: ImageFeatureExtractor,
    ...
    ckpt = {
        ...
        'last_n_img_stages': last_n_img_stages,
        'fusion': {
    ...
    # 保存已解冻的 ConvNeXt 顶层 stages；若为 ResNet 则保存 layer4
    if hasattr(image_extractor.model, 'stages'):
        total_stages = len(image_extractor.model.stages)
        start_idx_img = max(0, total_stages - max(1, last_n_img_stages))
        for i in range(start_idx_img, total_stages):
            ckpt['image_unfrozen'][f'stage_{i}'] = image_extractor.model.stages[i].state_dict()
    elif hasattr(image_extractor.model, 'layer4'):
...
def load_unfreeze_checkpoint(model: CrossModalRetrievalModel, text_extractor: TextFeatureExtractor, image_extractor: ImageFeatureExtractor,
    ...
    ln_img = ckpt.get('last_n_img_stages', last_n_img_stages)
    unfreeze_text_top_layers(text_extractor, last_n_layers=ln)
    unfreeze_image_top_block(image_extractor, last_n_img_stages=ln_img, unfreeze_layer4=True)
    enc = text_extractor.model.encoder
    ...
    # 恢复已保存的 ConvNeXt stages 或 ResNet layer4
    if hasattr(image_extractor.model, 'stages'):
        for k, v in ckpt['image_unfrozen'].items():
            if k.startswith('stage_'):
                idx = int(k.split('_')[-1])
                total_stages = len(image_extractor.model.stages)
                if 0 <= idx < total_stages:
                    image_extractor.model.stages[idx].load_state_dict(v)
