...
# 简单特征记忆队列：用于跨批提供负样本（难例挖掘）
class FeatureMemoryQueue:
    def __init__(self, max_size: int, dim: int, device: torch.device):
    ...
    def add(self, feats: torch.Tensor):
        ...
        self.buffer = torch.cat([self.buffer, feats], dim=0)
    ...
    def get_sample(self, k: int) -> torch.Tensor:
        if self.size() == 0:
            return self.buffer
        if self.size() <= k:
            return self.buffer
        idx = torch.randperm(self.size(), device=self.device)[:k]
        return self.buffer.index_select(0, idx)
...
# 扩展版InfoNCE：在文本侧引入来自记忆队列的跨批负样本
def info_nce_with_memory(text_feats: torch.Tensor, image_feats: torch.Tensor, temp: float,
                         neg_memory: torch.Tensor) -> torch.Tensor:
    # 基础双向对比损失
    logits = torch.mm(text_feats, image_feats.t()) / temp
    labels = torch.arange(logits.size(0), device=logits.device)
    loss_t = torch.nn.functional.cross_entropy(logits, labels)
    loss_i = torch.nn.functional.cross_entropy(logits.t(), labels)
    base = (loss_t + loss_i) * 0.5
    # 文本->图像侧扩展负样本以增强难例影响
    if neg_memory is not None and neg_memory.numel() > 0:
        extended = torch.cat([image_feats, neg_memory], dim=0)
        ext_logits = torch.mm(text_feats, extended.t()) / temp
        ext_labels = torch.arange(text_feats.size(0), device=text_feats.device)
        ext_loss_t = torch.nn.functional.cross_entropy(ext_logits, ext_labels)
        # 混合：保持基础双向，同时强化文本侧扩展负样本
        return 0.5 * base + 0.5 * ext_loss_t
    return base

# 基于记忆队列的难例挖掘：为每个文本从队列中选 top-k 难负样本，
# 使用 margin-ranking 风格的附加损失来增强对难负样本的区分。
def hard_negative_margin_loss(t_proj: torch.Tensor, i_proj: torch.Tensor, img_queue: torch.Tensor,
                              hard_k: int = 64, margin: float = 0.0) -> torch.Tensor:
    # t_proj: [B, D], i_proj: [B, D], img_queue: [M, D]
    if img_queue is None or img_queue.numel() == 0:
        return torch.zeros((), device=t_proj.device)
    # 正样本相似度（对角）
    pos_sims = torch.sum(t_proj * i_proj, dim=1)  # [B]
    # 队列负样本相似度
    neg_sims = t_proj @ img_queue.t()  # [B, M]
    # 选每行 top-k 难负样本
    k = min(hard_k, neg_sims.size(1))
    topk_vals, _ = torch.topk(neg_sims, k=k, dim=1, largest=True, sorted=False)  # [B, k]
    # margin-ranking: max(0, margin + neg - pos)
    # 广播 pos 到 [B, k]
    pos_expanded = pos_sims.unsqueeze(1).expand_as(topk_vals)
    relu = torch.nn.functional.relu(margin + topk_vals - pos_expanded)
    return relu.mean()
...
# 难例挖掘与记忆队列参数（训练用）
use_hard_mining = True            # 启用难例挖掘
hard_k = 64                       # 每个样本挑选的难负样本数量
hard_weight = 0.7                 # 难例附加损失的权重（提高以强化效果）
hard_margin = 0.05                # 难例margin值（非零以提升区分度）
memory_queue_size = 8192          # 负样本记忆队列大小（更大提升覆盖）
neg_queue_sample_size = 2048      # 每步从队列采样用于计算的负样本数量上限
use_queue_in_nce = True           # 在InfoNCE中引入记忆队列负样本（文本侧）
...
# 初始化图像特征记忆队列（用于难例挖掘）
img_neg_queue = FeatureMemoryQueue(
    max_size=memory_queue_size,
    dim=model.fusion.projection_dim if hasattr(model.fusion, 'projection_dim') else 512,
    device=device
)
...
def train_one_batch(pairs: List[Tuple[str, Image.Image, str]], img_neg_queue: FeatureMemoryQueue, epochs: int, step_bs: int):
        ...
        for s in range(0, len(pairs), step_bs):
            ...
            if use_amp and device.type == 'cuda':
                with autocast(enabled=True):
                    ...
                    if use_queue_in_nce and img_neg_queue.size() > 0:
                        neg_feats = img_neg_queue.get_sample(neg_queue_sample_size)
                        loss = info_nce_with_memory(t_proj, i_proj, temp=temperature, neg_memory=neg_feats)
                    else:
                        loss = info_nce_loss(t_proj, i_proj, temp=temperature)
                    if use_hard_mining and img_neg_queue.size() > 0:
                        neg_feats = img_neg_queue.get_sample(neg_queue_sample_size)
                        hn_loss = hard_negative_margin_loss(t_proj, i_proj, neg_feats, hard_k=hard_k, margin=hard_margin)
                        loss = loss + hard_weight * hn_loss
                ...
                scaler.update()
                scheduler.step()
                # 在参数更新后，将当前批次的图像投影特征加入队列，避免将当前正样本当作负样本
                if use_hard_mining:
                    img_neg_queue.add(i_proj.detach())
            else:
                t_feats = text_extractor.encode_with_grad(texts)
                i_feats = image_extractor.encode_with_grad(imgs)
                t_proj = model._norm(model.fusion.fuse_text_features(t_feats))
...
for image_batch in loader.load_images_batch(split='train', batch_size=train_image_batch_size, max_batches=max_train_batches):
    ...
    if not pairs:
        ...
        continue
    train_one_batch(pairs, img_neg_queue, epochs=epochs_per_batch, step_bs=train_step_batch_size)
    del img_map
    if device.type == 'cuda':
        torch.cuda.empty_cache()
