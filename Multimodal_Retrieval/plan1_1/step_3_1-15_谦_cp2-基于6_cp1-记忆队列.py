...
# 使用记忆队列扩展 InfoNCE 的图像负样本：
# - 文本->图像侧：在列拼接队列负样本；正样本仍位于前 B 列
# - 图像->文本侧：保持原始批内计算，避免需要文本队列
def info_nce_with_memory(t_proj: torch.Tensor, i_proj: torch.Tensor, temperature: float,
                         img_memory: FeatureMemoryQueue = None, neg_k: int = 1024) -> torch.Tensor:
    B = t_proj.size(0)
    if img_memory is not None and img_memory.size() > 0:
        mem = img_memory.get_sample(neg_k)
        # 假设加入队列的是已归一化的投影特征；若不是，也可再归一化
        all_i = torch.cat([i_proj, mem], dim=0)  # [B+M, D]
        logits_t = (t_proj @ all_i.t()) / temperature  # [B, B+M]
    else:
        logits_t = (t_proj @ i_proj.t()) / temperature  # [B, B]
    labels_t = torch.arange(B, device=t_proj.device)
    loss_t = torch.nn.functional.cross_entropy(logits_t, labels_t)
    # 图像->文本保持批内对称项
    logits_i = (i_proj @ t_proj.t()) / temperature  # [B, B]
    labels_i = torch.arange(B, device=i_proj.device)
    loss_i = torch.nn.functional.cross_entropy(logits_i, labels_i)
    return (loss_t + loss_i) * 0.5
...
# 记忆队列参数（适度激进）：跨批提供额外负样本
use_memory_loss = True           # 启用基于记忆队列的 InfoNCE 扩展
memory_queue_size = 8192         # 队列容量（略大以增强跨批覆盖）
negatives_k = 2048               # 每步拼接到文本-图像 logits 的队列负样本上限
...
def train_one_batch(pairs: List[Tuple[str, Image.Image, str]], epochs: int, step_bs: int, img_memory: FeatureMemoryQueue = None):
        ...
        for s in range(0, len(pairs), step_bs):
            ...
            if use_amp and device.type == 'cuda':
                with autocast(enabled=True):
                    ...
                    if use_memory_loss and img_memory is not None and img_memory.size() > 0:
                        loss = info_nce_with_memory(t_proj, i_proj, temperature=temperature, img_memory=img_memory, neg_k=negatives_k)
                    else:
                ...
                scaler.update()
                scheduler.step()
                # 更新记忆队列（在参数更新后加入当前批的图像投影特征）
                if use_memory_loss and img_memory is not None:
                    img_memory.add(i_proj.detach())
            else:
                t_feats = text_extractor.encode_with_grad(texts)
                i_feats = image_extractor.encode_with_grad(imgs)
                t_proj = model._norm(model.fusion.fuse_text_features(t_feats))
...
for image_batch in loader.load_images_batch(split='train', batch_size=train_image_batch_size, max_batches=max_train_batches):
    ...
    if not pairs:
        ...
        continue
    train_one_batch(pairs, epochs=epochs_per_batch, step_bs=train_step_batch_size, img_memory=img_memory)
    del img_map
    if device.type == 'cuda':
        torch.cuda.empty_cache()
